$(document).ready(function(){
  ...
  function buildCard(story){
    ...
    if(story.text!=undefined) {
    	...
    	$comments.forEach(function(){
    		$.ajax({
		      url: "https://hacker-news.firebaseio.com/v0/item/" + $comments + ".json",
		      dataType: 'JSON',
		      type: 'get',
  ...
  function buildComments(kids){
  	var html = '<div class="comment">';
  	if(kids!=undefined){
	  	var timeStamp = new Date(kids.time * 1000);
  		var humanTime = timeStamp.toUTCString();
	  	html += '<div class="date-author">' + kids.by + ' | ' + humanTime + '</div>';
	  	html += kids.text;
	  	html += '</div>';
	}
	if(kids.parent) {
